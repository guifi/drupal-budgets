 ...
 * to use:
 *
 * 1.- within a terminal shel, locate at working directory:
 *   cd <drupal_home_dirwctory>/sites/all/modules/budgets/quoteGrabbers/landatel
 * 2.- create a directory to download the images:
 *   mkdir ../images
 * 3.- execute grabber:
 *   php gLandatel.php > landatel.txt
 * 4.- within a browser, goto address for supplier quotes batch upload and
 *   select landatel.txt as the csv file, with | as a delimiter
 * 5.- within a terminal shell, copy all images to drupal files/catalog direcory:
 *   cp ../images/* <drupal_files_directory>/files/catalog
 *
 */
 ...
 foreach ( $html->find ( 'table[class=productListing] tr' ) as $product ) {
  ...
  foreach ( $productsTDs as $productTD ) {
   if ($productTD->class == 'productListing-heading')
    continue;
   if (!empty($productTD->find ( 's', 0 )))
     $productTD->find ( 's', 0 )->innertext = '';
   if (!empty($productTD->plaintext))
